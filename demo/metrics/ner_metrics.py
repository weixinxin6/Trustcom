...
class SeqEntityScore(object):
    ...
    def weight_compute(self, class_info, class_weight):
        recall = 0
        precision = 0
        f1 = 0

        for class_ in class_info:
            eval_res = class_info[class_]
            precision += eval_res['acc'] * class_weight[class_]
            recall += eval_res['recall'] * class_weight[class_]
            f1 += eval_res['f1'] * class_weight[class_]
        return recall, precision, f1
    ...
    def result(self):
        ...
        for type_, count in origin_counter.items():
            ...
            all_origin_count += count
            class_weight[type_] = count
        for class_ in class_weight.keys():
            class_count = class_weight[class_]
            class_weight[class_] = class_count/all_origin_count

        origin = len(self.origins)
        found = len(self.founds)
        right = len(self.rights)
        # recall, precision, f1 = self.compute(origin, found, right)
        recall, precision, f1 = self.weight_compute(class_info, class_weight)
