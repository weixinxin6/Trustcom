...
#model_type：'bert'、'bert_bilstm_crf'、bert_gru_crf(实际上是xlnet模型)
...
python method/xlnet_based.py \
  --task_name=aptner \
  --model_type=bert_gru_crf \
  --model_name_or_path=method/xlnet_cased_L-12_H-768_A-12/ \
  --data_dir=Data/APTNER/ \
  --output_dir=outputs/APTNER_xlnet/ \
  --markup=bioes \
  --language=en \
  --do_train \
  --do_eval \
